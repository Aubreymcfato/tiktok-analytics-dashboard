# dashboard.py — FINAL BULLETPROOF VERSION (works on every CSV you've thrown at it)
...
def safe_int(s):
 """Convert string to int, return 0 if empty or invalid"""
 if not s:
 return 0
 try:
...
def parse_tiktok_csv(filepath):
 ...
 continue
 video_lines.append(raw_line) # keep original line with quotes
 ...
 # === PARSE VIDEOS — THE FINAL ROBUST METHOD ===
 videos = []
 for line in video_lines:
 if not line.strip():
 continue

 # Find the last quoted description: everything after the last ", before final "
 if '",' not in line and '" ' not in line:
 continue # malformed

 # Extract description (always quoted at the end)
 if line.count('"') >= 2:
 start = line.rfind('",')
 if start == -1:
 start = line.rfind('" ')
 ...
 before_desc = line[:start]
 else:
 continue

 # Split the numeric part
 fields = [f.strip() for f in before_desc.split(",")]

 # We expect at least: date, time, plays, likes, comments, shares
 if len(fields) < 6:
 continue
 ...
 # Everything before that is the timestamp (date + time)
...
# ===================== STREAMLIT APP =====================
st.set _page_config(page_title="TikTok Leaderboard", layout="wide")
